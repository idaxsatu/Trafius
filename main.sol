...
contract Trafius {
    ...
    function probeDesk_33(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_34(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_35(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_36(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_37(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_43(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_44(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_45(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_46(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_47(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_48(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_49(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_50(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_51(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_52(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_53(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_54(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_55(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_56(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function probeDesk_57(uint256 deskId) external view returns (
        uint256 epochId,
        uint256 carryBps,
        uint256 capWei,
        uint256 staked,
    ...
    function batchRate_5(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 11) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_6(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 12) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_7(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 13) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_8(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 14) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_9(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 15) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_10(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 16) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_11(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 17) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_12(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 18) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_13(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 19) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_14(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 20) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_15(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 21) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_16(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 22) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_17(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 23) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function batchRate_18(uint256[] calldata slots, uint256[] calldata bps) external onlyDirector {
        if (slots.length != bps.length) revert TRF_SizeMismatch();
        if (slots.length > 24) revert TRF_BatchWide();
        for (uint256 i; i < slots.length; ++i) {
            if (bps[i] > TRF_MAX_MARGIN_BPS) revert TRF_RateHigh();
            rateSlotBps[slots[i]] = bps[i];
            emit RateSet(slots[i], bps[i], uint64(block.timestamp));
    ...
    function readRate_0(uint256 slot) external view returns (uint256 bps, uint256 echo) {
        bps = rateSlotBps[slot];
        echo = slot ^ 3;
    }

    function readRate_1(uint256 slot) external view returns (uint256 bps, uint256 echo) {
        bps = rateSlotBps[slot];
        echo = slot ^ 20;
    }

    function readRate_2(uint256 slot) external view returns (uint256 bps, uint256 echo) {
        bps = rateSlotBps[slot];
        echo = slot ^ 37;
    }

    function readRate_3(uint256 slot) external view returns (uint256 bps, uint256 echo) {
        bps = rateSlotBps[slot];
        echo = slot ^ 54;
    }
